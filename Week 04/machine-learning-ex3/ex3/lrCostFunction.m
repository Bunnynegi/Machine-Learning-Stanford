...
h_off_x = sigmoid(X * theta);
J = 1 / m * sum( -1 * y' * log(h_off_x) - (1-y') * log(1 - h_off_x) );
grad = 1 / m * (X' * (h_off_x - y));  %'
...
theta_zerofirst = [0; theta(2:length(theta));];
J = J + lambda / (2 * m) * sum( theta_zerofirst .^ 2 );
grad = grad .+ (lambda / m) * theta_zerofirst;
